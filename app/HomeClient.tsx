...
export default function HomeClient({ initialData }: HomeClientProps) {
  ...
  const { darkMode, autoRefresh } = useTheme();

  const freshness = useDataFreshness(data?.generated_at, autoRefresh);
  ...
  return (
    <div className={`min-h-screen ${bgClass} ${textClass}`}>
      ...
      {freshness.hasNewData && (
        <NewStoriesToast
          storyDelta={freshness.storyDelta}
          newTotal={freshness.newTotal}
          onRefresh={() => {
            freshness.dismiss();
            router.refresh();
          }}
          onDismiss={freshness.dismiss}
        />
      )}
    </div>
  );
