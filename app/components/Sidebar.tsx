...
export function Sidebar() {
  ...
  const { count: savedCount } = useSavedStories();
    ...
    <aside className={`fixed left-0 top-0 h-screen w-64 border-r hidden lg:flex flex-col p-4 gap-2 z-50 ${darkMode ? "border-zinc-800 bg-zinc-950" : "border-zinc-200 bg-zinc-50"}`}>
      ...
      <nav className="flex flex-col gap-1 flex-1">
        ...
        <a className={getLinkClass("/saved")} href="/saved">
          ...
          <span className="font-medium text-sm flex-1">Saved Items</span>
          {savedCount > 0 && (
            <span className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-bold ${darkMode ? "bg-indigo-500/20 text-indigo-300" : "bg-indigo-100 text-indigo-700"}`}>
              {savedCount}
            </span>
          )}
        </a>
